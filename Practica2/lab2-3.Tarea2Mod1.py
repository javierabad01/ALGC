...
def monte_carlo(matriz, n, m):
    ...
    for i in range(paracaidistas):
        ...
        calculo_matriz= h(-4 + random_y * 12 / (n-1), 8 + random_x * -12  / (m-1))
        matriz[random_x][random_y] = calculo_matriz
        punto_aleatorio = calculo_matriz

        trayectoria_X, trayectoria_Y = hill_climbingiteratorrativo(matriz, random_x, random_y, punto_aleatorio, [random_x], [random_y])
...
def hill_climbingiteratorrativo(matriz, x, y, punto, puntos_X, puntos_Y):
    while(True):
        array_colindantes = getColindantes(matriz, x, y)
        max_lista = np.max(list(array_colindantes.keys()))
        max_coor = array_colindantes[max_lista]
...
def getColindantes(matriz ,x,y):
    ...
    while(iterator<2):
        if(0<=x<n):
            calculo_matriz= h(-4 + y * 12 / (n-1), 8 + x * -12  / (m-1))
            matriz[x][y] = calculo_matriz
            dict[calculo_matriz] = [x,y]
        x += 2
        iterator += 1
    ...
    while(iterator<2):
        if(0<=y<m):
            calculo_matriz= h(-4 + y * 12 / (n-1), 8 + x_orig * -12  / (m-1))
            matriz[x_orig][y] = calculo_matriz
            dict[calculo_matriz] = [x_orig,y]
        y += 2
        iterator += 1
